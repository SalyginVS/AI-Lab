...
### 2.1a. vLLM MTP lane как альтернативный inference backend (ADR-027, v1.20)

**Решение.** В целевую архитектуру добавляется второй backend lane рядом с Ollama: `vLLM MTP` для speculative decoding / Multi-Token Prediction (MTP — Multi-Token Prediction, предсказание нескольких токенов за один шаг).

**Назначение lane.** Ускоренный bounded executor для коротких controlled coding задач в Continue. Lane не заменяет Ollama и не становится default Agent backend.
...
- `num_ctx=4096` / `max_model_len=4096`;
- `max_tokens=512`;
- `reasoning_effort=none`;
- роль: controlled bounded executor candidate;
- autonomous production changes запрещены;
- любые gateway/systemd/Docker/CI-CD изменения требуют отдельного approval gate.

**Причина.** MTP lane полезен как performance experiment, но его малое context window и новый backend-контур требуют отдельного role contract. Ускорение не должно автоматически повышать модель до autonomous agent.
...
### 7.15. Runtime filesystem is not documentation source of truth (v1.20)

Серверный runtime-контур не является каноническим Git/knowledge контуром. Runtime-директории могут содержать рабочий код и конфигурации сервисов, но не должны быть местом принятия архитектурных решений, паспортов, ADR или stage results.
...
**Следствие:** любые изменения runtime должны финализироваться через Obsidian/GitHub-документацию: паспорт, целевая архитектура, ADR, грабли, этапы. Server-side Git remotes для `llm-gateway` удалены как ошибочный knowledge contour.

---

### 7.16. Bounded acceleration lanes do not imply Agent readiness (ADR-027, v1.20)

Speculative decoding / MTP / alternative runtime backends повышают throughput или latency profile, но не являются доказательством:

- instruction-following discipline;
- safe tool use;
- autonomous multi-file agent readiness;
- production change readiness.

Promotion path для таких lane: bounded task → controlled executor → IDE validation → policy/tool boundary → только затем agent candidate.
...
| **2026-05-08** | **Qwen3.6 27B MTP / Continue integration + source-of-truth consolidation** | **v1.20: добавлен альтернативный vLLM MTP backend lane для `qwen3.6-27b-lorbus-mtp-triton-ctx4k` через gateway provider `vllm-mtp`; роль lane ограничена controlled bounded executor candidate; autonomous agent readiness explicitly rejected. Зафиксировано правило: server runtime filesystem не является documentation source of truth; `/home/vladimir/llm/AI-Lab` + GitHub — единственный канонический knowledge contour. ADR-027, Грабли #78–81.** |
...
| MTP | Multi-Token Prediction — speculative decoding подход, при котором модель/связка моделей предсказывает несколько токенов за шаг для ускорения генерации |
| vLLM | Высокопроизводительный inference engine для LLM с OpenAI-compatible API и optimized serving |
...
## Дополнение v1.21 — OpenCode controlled terminal-agent lane

### Решение

В целевую архитектуру добавлен controlled terminal-agent lane на базе OpenCode v1.15.0.

Контур применения:

```text
Ubuntu laptop / VS Code-adjacent terminal
  -> OpenCode wrapper
  -> llm-gateway http://192.168.0.128:8000/v1
  -> qwen3.6:27b
  -> AI-Lab documentation repo
```

### Роль

OpenCode не заменяет Continue.dev как primary IDE agent layer. Его роль ограничена терминальным controlled execution lane для:
...
- bash-enabled mode;
- task/subagent-enabled mode;
- web-enabled mode;
- external_directory access;
- `edit=ask` in scripted run;
- `--dangerously-skip-permissions`.

### Architecture principle

Terminal-agent capability does not imply autonomous-agent readiness.

Promotion requires separate validation gate and stronger telemetry / policy enforcement.
