...
**Паспорт стенда:** v28.0 (2026-04-10)
...
| 1. Inference/Backend | Gemma 4: gemma4:31b (Quality-first Agent/Planner/Reviewer/Best Semantic SQL) | **Active** | 10C, F-next ✅ |
...
- 12 моделей (после ревизии R: 17→12, удалены 5 reserve/legacy)
- OLLAMA_MAX_LOADED_MODELS=2, NUM_PARALLEL=1, Flash Attention, KV (Key-Value) cache q8_0

**Что изменилось в 10C–R:**
- Ollama 0.18.0 → 0.20.2 (Gemma 4, security patches)
- 13 → 16 → 15 → 17 → 12 моделей (+ Gemma 4, − deprecated, − reserve при ревизии R)
...
**Решение по маршрутизации моделей (ADR-012, обновлено после ревизии R):**

**Tier 1 — Primary workhorses:**

| Роль | Модель | Путь | num_ctx | reasoning_effort | Этап |
|------|--------|------|---------|-----------------|------|
| Quality-first Agent / Planner / Reviewer / **Best Semantic SQL** | **gemma4:31b** | шлюз | 8192 (до 256K) | none / high | 10C, **F-next** |
| Fast Semantic Agent / Fast Executor | **gemma4:26b** (MoE, 3.8B active) | шлюз | 8192 (до 256K) | none | 10C |
| Stable Generic Executor / Backup Agent / **Best Plain SQL Executor** | qwen3-coder:30b | шлюз | 8192 | none | 7A, **11A** |
...
| L1 — Canonical | Official architecture decisions (ADR-008…**018**) | **9** | Primary index |
...
```
Параллельные треки (обновлено v1.14):
...
ВСЕ ТРЕКИ ЗАВЕРШЕНЫ. Ревизия R✅ выполнена.
...
| **Onboarding** | Паспорт (✅ v28) | **ONBOARDING.md + setup-check 66 проверок (✅ 13+R)** | Полный onboarding < 30 мин |
...
| 20 | gemma4:31b как unified Planner+Reviewer | [A] Не тестировалось | Высокое | Будущее |
...
| **2026-04-10** | **Этап F-next + Ревизия R** | **v1.14: Semantic Layer (SA 90%, ADR-018). Все треки A–F завершены. Tech debt sweep: 17→12 моделей, health/setup checks обновлены, sql_knowledge синхронизирован, RAG corpus +ADR-017/018 (14 files/18 chunks). Dual orchestration path зафиксирован. Ollama 0.20.4 FA bug заблокировал upgrade. Continue pivot отмечен как Weak Signal. Открытые вопросы #22–23 закрыты, #28–35 добавлены. Roadmap из v1.0 полностью реализован.** |
